...
from .widgets import CloudflareVideoUploadWidget, MediaCreationWidget, VideoStatusWidget
...
class TrailerMetaAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            if self.instance.cf_video_uid:
                ...
                self.fields["video_upload"].widget.attrs[
                    "data-current-uid"
                ] = self.instance.cf_video_uid
        ...
        self.fields["video_number"].help_text = (
            "Sequential number for organizing videos"
        )
    ...
    def clean_price(self):
        ...
        if not re.match(r"^\$\d+(\.\d{2})?$", price):
            raise ValidationError(
                "Price must be in format '$20' or '$20.00', or 'FREE'"
            )
    ...
    def clean_length(self):
        ...
        if not any(
            re.match(pattern, length, re.IGNORECASE) for pattern in valid_formats
        ):
            raise ValidationError(
                "Length must be in format '25 Minutes' or '1 Hour 15 Minutes'"
            )
        ...
        if video_upload and hasattr(video_upload, "file"):
            try:
                ...
                upload_result = cf_service.upload_video(
                    ...
                    metadata={
                        "name": (
                            ...
                            if instance.media
                            else f"Video {instance.video_number}"
                        ),
                ...
                if upload_result["success"]:
                    ...
                    logger.info(
                        f"Successfully uploaded video to Cloudflare: {instance.cf_video_uid}"
                    )
                else:
                    raise ValidationError(
                        f"Failed to upload video: {upload_result['error']}"
                    )
...
class VideoStatusForm(forms.Form):
    ...
    video_uid = forms.CharField(max_length=32, help_text="Cloudflare Stream video UID")
...
class CloudflareSettingsForm(forms.Form):
    ...
    account_id = forms.CharField(max_length=100, help_text="Cloudflare Account ID")
