...
class TrailerMeta(models.Model):
    """
    Extended metadata for trailer videos in MediaCMS
    Links to MediaCMS Media model and adds premium content fields
    """
    
    # Link to MediaCMS Media model
    ...
    creators = models.CharField(
        max_length=200,
        help_text="Content creators/performers"
    )
    ...
    is_featured = models.BooleanField(
        default=False,
        help_text="Feature this trailer prominently"
    )
    is_premium = models.BooleanField(
        default=True,
        help_text="Requires payment/subscription to view"
    )
    ...
    def get_duration_minutes(self):
        ...
        match = re.search(r'(\d+)\s*(?:Hour|Hr)?s?\s*(\d+)?\s*(?:Minute|Min)?s?', self.length, re.IGNORECASE)
        if match:
            hours = int(match.group(1)) if 'hour' in self.length.lower() else 0
            minutes = int(match.group(2)) if match.group(2) else (int(match.group(1)) if 'hour' not in self.length.lower() else 0)
