...
from .forms import BulkUploadForm, CloudflareSettingsForm, VideoStatusForm
from .models import TrailerMeta
from .services import VideoUploadHandler, get_cloudflare_service
...
def upload_video(request):
    ...
    try:
        ...
        if video_file.size > max_size:
            return JsonResponse(
                {"success": False, "error": "File size must be less than 2GB"}
            )
        ...
        if upload_result["success"]:
            ...
            return JsonResponse({"success": False, "error": upload_result["error"]})
...
def create_media(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": f"Media creation failed: {str(e)}"}
        )
        ...
        if form.is_valid():
            try:
                ...
                for row in csv_reader:
                    try:
                        video_number = int(
                            row.get("Video Number", "0").replace("Video ", "")
                        )
                        ...
                        if not dry_run:
                            ...
                            if update_existing:
                                try:
                                    existing = TrailerMeta.objects.get(
                                        video_number=video_number
                                    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                return render(
                    request, "admin/trailers/video_status_check.html", context
                )
...
def trailer_dashboard(request):
    ...
    try:
        ...
        completed_trailers = TrailerMeta.objects.filter(
        ...
        context = {
            ...
            "completion_rate": (
                (completed_trailers / total_trailers * 100) if total_trailers > 0 else 0
            ),
        }
