    ...
    def process_row(self, row, existing_media, dry_run):
        ...
        if title_upper in existing_media:
            ...
            if best_match:
                media = best_match
                self.stdout.write(
                    f"  Fuzzy match for Video {video_number}: {similarity:.2%}"
                )

        if not media:
            self.stdout.write(
                f"  Skipping Video {video_number} - No matching Media object found"
            )
            return "skipped"
        ...
        if TrailerMeta.objects.filter(media=media).exists():
            self.stdout.write(
                f"  Skipping Video {video_number} - TrailerMeta already exists"
            )
            return "skipped"
        ...
        if dry_run:
            self.stdout.write(
                f"  WOULD LINK: Video {video_number} -> Media ID {media.id} - {title[:50]}..."
            )
            return "linked"
        ...
        self.stdout.write(
            f"  Linked: Video {video_number} -> Media ID {media.id} - {title[:50]}..."
        )
        return "linked"
