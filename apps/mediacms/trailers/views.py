from django.db.models import Q, Case, When, Value, FloatField, IntegerField
from django.db.models.functions import Cast, Substr, Length, Extract
...
class TrailerMetaFilter(django_filters.FilterSet):
    ...
    def filter_price_min(self, queryset, name, value):
        """Filter by minimum price using database-level operations"""
        return queryset.annotate(
            price_numeric=Case(
                When(price__iexact='FREE', then=Value(0.0)),
                When(price__iexact='$0', then=Value(0.0)),
                # Extract numeric value from price strings like "$20"
                default=Cast(
                    Substr('price', 2),  # Remove $ symbol
                    FloatField()
                ),
                output_field=FloatField()
            )
        ).filter(price_numeric__gte=value)

    def filter_price_max(self, queryset, name, value):
        """Filter by maximum price using database-level operations"""
        return queryset.annotate(
            price_numeric=Case(
                When(price__iexact='FREE', then=Value(0.0)),
                When(price__iexact='$0', then=Value(0.0)),
        ...
        ).filter(price_numeric__lte=value)

    def filter_length_min(self, queryset, name, value):
        """Filter by minimum duration using database-level operations"""
        # Handle "25 Minutes" and "1 Hour 15 Minutes" formats
        return queryset.extra(
            where=["""
                CASE 
                    WHEN LOWER(length) LIKE '%%hour%%' THEN
                        COALESCE(
                            CAST(SUBSTRING(length FROM '(\\d+)\\s*(?:Hour|Hr)') AS INTEGER) * 60, 0
                        ) + 
                        COALESCE(
                            CAST(SUBSTRING(length FROM '(?:Hour|Hr)\\s*(?:\\w+\\s+)?(\\d+)\\s*(?:Minute|Min)') AS INTEGER), 0
                        )
                    ELSE
                        COALESCE(
                            CAST(SUBSTRING(length FROM '(\\d+)\\s*(?:Minute|Min)') AS INTEGER), 0
                        )
                END >= %s
            """],
            params=[value]
    ...
    def filter_length_max(self, queryset, name, value):
        """Filter by maximum duration using database-level operations"""
        # Handle "25 Minutes" and "1 Hour 15 Minutes" formats
        return queryset.extra(
            where=["""
                CASE 
