...
class TrailerMetaAdmin(admin.ModelAdmin):
    ...
    def refresh_video_status(self, request, queryset):
        ...
        try:
            ...
            self.message_user(
                request, f"Refreshed status for {updated_count} trailer(s)."
            )

        except Exception as e:
            self.message_user(
                request, f"Error refreshing status: {str(e)}", level=messages.ERROR
            )
                ...
                if trailer.cf_video_uid:
                    ...
                    metadata = {
                        "name": (
                            trailer.media.title
                            if trailer.media
                            else f"Video {trailer.video_number}"
                        ),
                        "description": (
                            trailer.detailed_description or trailer.media.description
                            if trailer.media
                            else ""
                        ),
                    }

                    result = cf_service.update_video_metadata(
                        trailer.cf_video_uid, metadata
                    )
                    if result["success"]:
                        synced_count += 1

            self.message_user(
                request, f"Synced metadata for {synced_count} trailer(s)."
            )

        except Exception as e:
            self.message_user(
                request, f"Error syncing metadata: {str(e)}", level=messages.ERROR
            )
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                ...
                name="trailers_bulk_upload",
            ),
            path(
                "dashboard/",
                self.admin_site.admin_view(self.dashboard_view),
                name="trailers_dashboard",
            ),
            path(
                "video-status-check/",
                self.admin_site.admin_view(self.video_status_check_view),
                name="trailers_video_status_check",
            ),
            path(
                "cloudflare-settings/",
                self.admin_site.admin_view(self.cloudflare_settings_view),
                name="trailers_cloudflare_settings",
            ),
        ]
