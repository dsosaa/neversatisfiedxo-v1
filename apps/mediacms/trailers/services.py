...
class CloudflareStreamService:
    ...
    def get_video_status(self, video_uid: str) -> Dict:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/{video_uid}", headers=self.headers
            )
            response.raise_for_status()
    ...
    def wait_for_processing(
        self, video_uid: str, max_wait: int = 300, check_interval: int = 10
    ) -> Dict:
        """
        ...
        while time.time() - start_time < max_wait:
            ...
            logger.info(
                f"Video {video_uid} still processing, status: {status['status']}"
            )
    ...
    def delete_video(self, video_uid: str) -> Dict:
        ...
        try:
            ...
            if data.get("success"):
                ...
                return {"success": False, "error": f"Failed to delete video: {data}"}
    ...
    def update_video_metadata(self, video_uid: str, metadata: Dict) -> Dict:
        ...
        try:
            ...
            if data.get("success"):
                ...
                return {"success": False, "error": f"Failed to update metadata: {data}"}
...
class VideoUploadHandler:
    ...
    def process_upload(
        self, video_file: BinaryIO, trailer_data: Dict
    ) -> Tuple[bool, Dict]:
        """
        ...
        try:
            ...
            if (
                not status["success"]
                and "timeout" not in status.get("error", "").lower()
            ):
