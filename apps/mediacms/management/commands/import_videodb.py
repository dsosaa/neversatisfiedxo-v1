...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            with open(csv_file, 'r', encoding='utf-8') as file:
                ...
                required_columns = [
                    'Video Number', 'Description', 'Price', 'Length', 
                    'Creators', 'Video ID', 'Thumbnail ID', 'Upload Status'
                ]
                ...
                for row_num, row in enumerate(reader, start=2):  # Start at 2 for header
                    try:
                        with transaction.atomic():
                            result = self.process_row(row, user, update_existing, dry_run)
    ...
    def process_row(self, row, user, update_existing, dry_run):
        ...
        if existing_trailer and not update_existing:
            self.stdout.write(f'  Skipping existing trailer: Video {video_number} ({cf_video_uid})')
            return 'skipped'
    ...
    def parse_duration(self, duration_str):
        ...
        match = re.search(r'(\d+)\s*(?:Hour|Hr)s?\s*(\d+)?\s*(?:Minute|Min)s?', duration_str, re.IGNORECASE)
        if match:
