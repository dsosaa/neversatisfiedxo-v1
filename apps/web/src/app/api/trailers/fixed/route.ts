...
async function getTrailers(): Promise<TrailerData[]> {
  ...
  try {
    ...
    trailerCache = records.map((row: any) => {
    ...
    lastCacheTime = now
    console.log(`✅ Processed ${trailerCache?.length || 0} trailers with ${Object.keys(VIDEO_ID_MAPPING).length} real video mappings`)
    return trailerCache || []
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    console.log(`🎯 API Request: returning ${paginatedTrailers.length} trailers (${workingVideos.length} working, ${brokenVideos.length} broken)`)
